...
public class MazerNoise : MonoBehaviour
{
    ...
    private float noiseFactor = 0.99f;
    ...
    void Start()
    {
        ...
        pixelGen.initPixels(40);
        pixelGen.noiseField(noiseFactor);

        pixelGen.mazeSurface = Instantiate(arenaPrefab, Vector3.zero, Quaternion.Euler(new Vector3(90, 0, 0)));
        ...
        pixelGen.mazeSurface.transform.localScale *= 300.0f;
        pixelGen.mazeSurface.GetComponent<Renderer>().material.mainTexture = pixelGen.PartialMaze(1);
        pixelGen.mazeSurface.SetActive(true);
        ...
        camera.transform.position = new Vector3(0, 5500, 0);
    ...
    void FixedUpdate()
    {
        ...
        pixelGen.PartialMaze(5);
        
        if (pixelGen.finished)
        {
            Destroy(pixelGen.mazeSurface);
            pixelGen = new PixelMazeGen();
            noiseFactor -= 0.01f;
            pixelGen.initPixels(40);
            pixelGen.noiseField(noiseFactor);
    ...
    void Update()
    {
        ...
        {
            addFlatMaze(new Vector3(Random.Range(-camera.transform.position.y, camera.transform.position.y), Random.Range(camera.transform.position.y / 2, camera.transform.position.y), Random.Range(-camera.transform.position.y, camera.transform.position.y)));
        }
    ...
    public void addFlatMaze(Vector3 location)
    {
